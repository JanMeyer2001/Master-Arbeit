...
def main():
    ...
    if os.path.exists('Quantitative_Results/'+model_dir[:-1]+'_Test_Bilinear.csv'):
        os.remove('Quantitative_Results/'+model_dir[:-1]+'_Test_Bilinear.csv')
    csv_writter(model_dir[:-1], 'Quantitative_Results/' + model_dir[:-1]+'_Test_Bilinear')
    line = ''
    csv_writter(line +','+'non_jec', 'Quantitative_Results/' + model_dir[:-1]+'_Test_Bilinear')
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            def_out = transform(x_seg_oh.float(), D_f_xy.permute(0, 2, 3, 4, 1))
            ...
            csv_writter(line, 'Quantitative_Results/' + model_dir[:-1]+'_Test_Bilinear')
