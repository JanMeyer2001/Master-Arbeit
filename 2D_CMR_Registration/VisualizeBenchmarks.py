...
foldername4 = 'Loss_1_Chan_8_Smth_0.01_LR_0.0001_Mode_3'
filename4 = foldername4 + '_Png/' + foldername4 + '.csv'
...
with open(filename4,'r') as csvfile: 
    ...
    for i, row in enumerate(lines): 
        if i>0:
            index4.append(int(row[0])) 
            MSE4.append(float(row[1])) 
            SSIM4.append(float(row[2]))
...
plt.plot(index4, MSE4, color = 'y', label = "Subsampled (Acc10)") 
...
plt.plot(index4, SSIM4, color = 'y', label = "Subsampled (Acc10)") 
plt.xlabel('Iterations') 
plt.ylabel('SSIM') 
plt.legend()

plt.savefig('/home/jmeyer/storage/students/janmeyer_711878/Master-Arbeit/Thesis/Images/ValidationBenchmark-' + foldername1 + '+Mode1+Mode2+Mode3.png')
plt.close()