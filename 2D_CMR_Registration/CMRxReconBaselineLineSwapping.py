...
for z in folders:
    ...
    for mode in modes:
        ...
        data_set = DatasetMotionReconstruction_LineSwapping('/home/jmeyer/storage/students/janmeyer_711878/data/CMRxRecon', False, mode, z) 
        ...
        for i, data in enumerate(data_generator):
            ...
            images_subsampled   = data[1].squeeze()     # tensor with size (1,F,H,W)
            ...
            num_frames = images_fullysampled.shape[0]   # number of frames F
            ...
            for frame in range(num_frames):
                ...
                csv_HaarPSI = haarpsi(images_subsampled[frame,:,:].unsqueeze(0).unsqueeze(0), images_fullysampled[frame,:,:].unsqueeze(0).unsqueeze(0), data_range=1).item()
                csv_PSNR    = psnr(images_subsampled[frame,:,:].unsqueeze(0).unsqueeze(0), images_fullysampled[frame,:,:].unsqueeze(0).unsqueeze(0), data_range=1).item()
                csv_SSIM    = ssim(images_subsampled[frame,:,:].unsqueeze(0).unsqueeze(0), images_fullysampled[frame,:,:].unsqueeze(0).unsqueeze(0), data_range=1).item()
                csv_MSE     = mean_squared_error(images_fullysampled[frame,:,:].cpu().detach().numpy(), images_subsampled[frame,:,:].cpu().detach().numpy())
