            ...
            if (step % n_checkpoint == 0):
                with torch.no_grad():
                    ...
                    for mov_img, fix_img, mov_lab, fix_lab in valid_generator:
                        ...
                        vdisp_mf_1 = torch.real(torch.fft.ifft2(torch.fft.ifftshift(vout_ifft1)))
                        vdisp_mf_2 = torch.real(torch.fft.ifft2(torch.fft.ifftshift(vout_ifft2)))
                        vf_xy = torch.cat([vdisp_mf_1.unsqueeze(0).unsqueeze(0), vdisp_mf_2.unsqueeze(0).unsqueeze(0)], dim = 1)
                        __, warped_xv_seg= transform(mov_lab.float().to(device), vf_xy.permute(0, 2, 3, 1), mod = 'nearest')
                    ...
                    modelname = 'DiceVal_{:.5f}_Step_{:06d}.pth'.format(np.mean(Dices_Validation),step)
